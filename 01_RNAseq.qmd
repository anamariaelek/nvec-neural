...
```{r}
norm_mt <- log10(con_mt+1)
```

Inspect genes normalized expression distribution

```{r fig.width=8, fig.height=6, warning=FALSE, message=FALSE}
# normalized accessibility distribution
norm_dt <- as.data.table(norm_mt, keep.rownames = "gene")
...
gene_order <- unlist(gene_order[cluster_order])
```
