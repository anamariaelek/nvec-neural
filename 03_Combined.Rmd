...
pou_nco_gen <- pou_net[sample=="Ncol_pos", .(target_gene, target_expression_lfc)]
pou_ela_gen <- pou_net[sample=="Elav_pos", .(target_gene, target_expression_lfc)]
setnames(pou_nco_gen, "target_expression_lfc", "ncol+")
setnames(pou_ela_gen, "target_expression_lfc", "elav+")
pou_dt <-
  merge.data.table(
    merge.data.table(
      merge.data.table(
      ...
      pou_nco_gen, by = "target_gene", all = TRUE
    ),
    pou_ela_gen, by = "target_gene", all = TRUE
  )

pou_mt <- pou_dt[, lapply(.SD, function(x) !is.na(x)), .SD = c("activated", "repressed", "ncol+", "elav+")]
pou_mt[, target_gene := pou_dt$target_gene]
pou_mt <- unique(pou_mt)
pou_mt[, network := sum(`ncol+`, `elav+`), by = .I]
setorder(pou_mt, -network)
setcolorder(pou_mt, c("target_gene", "network", "ncol+", "elav+", "activated", "repressed"))
pou_mt <- pou_mt[target_gene != pou_mt[activated==TRUE & repressed==TRUE]$target_gene]
mt <- as.matrix(pou_mt[, -c(1:2)])

require(eulerr)
fit <- euler(mt, weights = pou_mt$network + 1)
p1 <- plot(
  fit,
  quantities = TRUE,
  fills = list(fill = c(line_cols[c("Ncol", "Elav")], c("#92c5de", "#e5998d")))
)
print(p1)
...
pou_dt[comb %in% c("ncol","elav","ncol&elav"), comb_agg := "network only"]
pou_dt[grepl("&activated", comb), comb_agg := "network&activated"]
pou_dt[grepl("&repressed", comb), comb_agg := "network&repressed"]
pou_dt[, intersect := "no"][grepl("&", comb_agg), intersect := "yes"]
pou_dt[, comb_n := .N, comb]
comb_cols <- c(
  "ncol" = unname(line_cols["Ncol"]),
  "elav" = unname(line_cols["Elav"]),
  "ncol&elav" = "#b69e2eff",
  "activated" = "#92c5de",
  "repressed" = "#e5998d",
  "elav&activated" = "#dda47eff",
  "elav&repressed" = "#f58c55ff",
  "ncol&activated" = "#76ba95ff",
  "ncol&repressed" = "#a4a86cff",
  "ncol&elav&activated" = "#b4aa6dff",
  "ncol&elav&repressed" = "#c79d51ff",
  "network only" = "#878787",
  "network&activated" = "#2166ac",
  "network&repressed" = "#b2182b"
)

gp_cor <- ggplot(
  pou_dt,#[comb_n > 20][!comb %in% c("ncol","elav","ncol&elav")],
  aes(corl,color=comb_agg)
) + 
  geom_density() +
...
```{r include=FALSE, eval=TRUE}
pdf(
  file.path(fig_dir, "Pou4_targets_correlation_elife.pdf"),
  width = 6, height = 4
)
gp_cor
dev.off()
...
pou_pub_act <- pou_pub[regulation == "up", .(target_gene, fold_change)]
setnames(pou_pub_act, "fold_change", "up")
pou_pub_rep <- pou_pub[regulation == "down", .(target_gene, fold_change)]
setnames(pou_pub_rep, "fold_change", "down")
pou_gen <- pou_net[,.(target_gene, target_expression_lfc)]
...
pou_dt <-merge.data.table(
  ...
  pou_gen, by = "target_gene", all = TRUE
)

pou_mt <- pou_dt[, lapply(.SD, function(x) !is.na(x)), .SD = c("up", "down", "net")]
pou_mt[, target_gene := pou_dt$target_gene]
pou_mt <- unique(pou_mt)
setcolorder(pou_mt, c("target_gene", "net", "up", "down"))
mt <- as.matrix(pou_mt[, -c(1)])
...
p1 <- plot(
  fit,
  quantities = TRUE,
  fills = list(fill = c("#a4a86cff", "#92c5de", "#e5998d"))
)
print(p1)
...
pou_net <- pou_net[expression_lfc>1.5 & target_expression_lfc>1.5]
...
pou_dc <- pou_dc[!target_gene %in% pou_dc[!is.na(repressed) & !is.na(activated)]$target_gene]
...
# motif scores
#mona <- fread(file.path(atac_dir, "motif-scores-archetypes-mona-0.8-annot.tsv"))
#mona_pou <- unique(mona[grep("ARCH1151", motif)][gene != ""][, .(score, gene)])
#setorder(mona_pou, -score)
#mona_pou <- mona_pou[, .SD[1], gene]
#setnames(mona_pou, c("score", "gene"), c("motif_score_monalisa", "target_gene"))
#sub_dt <- merge.data.table(sub_dt, mona_pou, by = "target_gene", all.x = TRUE, sort=FALSE)
#sub_dt[!is.na(motif_score_monalisa)][,.N,is.na(motif_score)]
#sub_dt[, motif_score := motif_score_monalisa]
#sub_dt[, motif_score_monalisa := NULL]
...
Cell type level expression for different groups of Pou4 targets
...
ct_dt <- melt.data.table(as.data.table(
  ct_adult@mc_fp, keep.rownames = "target_gene"
), id.vars = "target_gene", variable.name = "cell_type", value.name = "mcfp")
gs_dt <- unique(sub_dt[, .(target_gene, regulation)])
ct_dt <- merge.data.table(
  ct_dt, gs_dt, by = "target_gene", sort = FALSE
)
# colors
...
# plot
ct_dt[, regulation := factor(regulation, levels = levels(sub_dt$regulation))]
ct_dt[, cell_type := factor(cell_type, levels = names(ct_cols))]
ct_dt[, mcfp := pmin(mcfp, 4)]
sub_reg <- c(
  "network",
  "network+down", "network+activated+down", "activated+down",
  "network+up", "network+up+repressed", "up+repressed"
)
ct_sub_dt <- ct_dt[regulation %in% sub_reg]
ct_sub_dt[, regulation := factor(regulation, levels = sub_reg)]
gp_ct <- ggplot(
    ct_sub_dt,
    aes(cell_type, mcfp)
  ) +
  geom_boxplot(aes(color = cell_type), outlier.size = 0.8) +
  geom_boxplot(aes(fill = cell_type), outlier.colour = NA) +
...
```{r include=FALSE, eval=TRUE}
pdf(
  file.path(fig_dir, "Pou4_targets_network_expression_cell_type.pdf"),
  width = 4, height = 12
)
gp_ct
dev.off()
