...
# save
fwrite(dp, file.path(
  res_dir, "motif-gene-correaltion-pairs-v2.tsv"
), sep = "\t")
...
pdf(
  file.path(fig_dir, "motifs-genes-correaltion-v2.pdf"),
  width = 8.5, height = 5
)
gp_cor
dev.off()
...
pks_list <- lapply(names(pks_files), function(x) {
    np <- consensusSeekeR::readNarrowPeakFile(
      pks_files[x],
      extractRegions = TRUE,
      extractPeaks = FALSE
    )$narrowPeak
...
pks_clusters <- unique(as.data.table(cpeaks_sm)[, .(name, sample)])
setnames(pks_clusters, c("peak", "cluster"))

smp <- unique(pks_clusters$cluster)
smp <- grep("pos", smp, value = TRUE)
smp_clusters <- sapply(smp, function(x) x)
```
...
# scanning
scores_list <- sapply(names(smp_clusters), function(sm) {
...
scores_dt <- rbindlist(scores_list, idcol = "sample")

# motif quantile scores
scores_q <- ecdf(scores_dt$motif_score)(scores_dt$motif_score)
scores_dt[, motif_score_quantile := scores_q]
...
scor_mo[, motif_score_quantile := range_01(motif_score)]

# plot
scor_mp <- copy(scor_mo)
scor_mp[, sample := str_extract(sample, "Elav|Fox|Ncol")]
gp_score <- ggplot(scor_mp, aes(motif_score_quantile, fill = sample)) +
...
((gp_tpm / gp_tpm_tfs / gp_score / gp_enr_fc / gp_enr_qv / gp_ftp) &
  theme(legend.position = "bottom")) +
  plot_layout(guides = "collect")
dev.off()
```
...
scor_mc <- structure(
  scor_mo[sample == sm]$motif_score_quantile,
  names = scor_mo[sample == sm]$gene
)
...
is_dt[, TPM_tg := tpm_tg[gene]][, TPM_tf := tpm_tf]
is_dt[, motif_quantile_score := scr_tf[gene]]
is_dt[, footprint_score := ftp_tf[gene]]
is_dt[, motif_enrichment_fc := enr_fc]
is_dt[, motif_enrichment_minuslog10_q_value := enr_qv]
...
fwrite(is_ann, file.path(res_dir, sprintf(
  "motif-%s-sample-%s-interaction-scores.tsv", mo, sm
)), sep = "\t")
```
...
```{r}
is_ann <- fread(file.path(res_dir, sprintf(
  "motif-%s-sample-%s-interaction-scores.tsv", mo, sm
)))
#is_ann <- is_ann[motif_quantile_score > 0]
...
gp_is <- ggplot(is_ann, aes(is)) +
  ...
  geom_density(aes(color = motif_quantile_score > 0)) +
  geom_rug(data = is_ann[flagellar == TRUE], sides = "b", alpha = 0.5) +
  scale_x_continuous(limits = c(0, 1)) +
  facet_wrap(. ~ group, labeller = interaction_labeller) +
...
gp_is_qs <- ggplot(
  is_ann[grep("qscore", group)],
  aes(is, motif_quantile_score)
) +
  geom_point(alpha = 0.6) +
  facet_wrap(. ~ group, scales = "free_y", labeller = interaction_labeller) +
  labs(x = "interaction score", y = "motif quantile score") +
  theme(panel.grid.major = element_line(linewidth = 0.1))

gp_is_fpm <- ggplot(
  is_ann[grep("footprint", group)],
  aes(is, TPM_tg)
) +
  geom_point(alpha = 0.6) +
  facet_wrap(. ~ group, scales = "free_y", labeller = interaction_labeller) +
  labs(x = "interaction score", y = "footprint score") +
  theme(panel.grid.major = element_line(linewidth = 0.1))
...
gp_is_tpm <- ggplot(
  ...
  geom_point(alpha = 0.6) +
  facet_wrap(. ~ group, scales = "free_y", labeller = interaction_labeller) +
  labs(x = "interaction score", y = "target gene TPM") +
  theme(panel.grid.major = element_line(linewidth = 0.1))
```
...
pdf(file.path(fig_dir, sprintf(
  "motif-%s-sample-%s-interaction-scores-correlation.pdf", mo, sm
)), width = 16, height = 14)
gp_is_qs
gp_is_ft
gp_is_tpm
...
is_ann[group == "footprint"][order(-is)]
is_ann[group == "qscore"][order(-is)]
is_ann[group == "footprint_qscore"][order(-is)]

is_dt <- unique(is_ann[, .(group, gene, is, flagellar)])
is_dt <- dcast.data.table(is_dt, gene + flagellar ~ group, value.var = "is")

gp_is_qs <- ggplot(
  is_dt,
  aes(footprint, qscore)
) +
  geom_point(alpha = 0.6) +
  theme(panel.grid.major = element_line(linewidth = 0.1))
```
...
```{r include=FALSE, eval=TRUE}
pdf(file.path(fig_dir, sprintf(
  "motif-%s-sample-%s-interaction-scores-comparison.pdf", mo, sm
)), width = 8, height = 8)
gp_is_qs
dev.off()
