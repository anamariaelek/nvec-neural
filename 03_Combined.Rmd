...
```{r}
# load data from paper
pou_pub_target_genes <- unique(fread(file.path(pub_dir, "pou4_targets_elife_all.tsv"))$target_gene)

# which genes from our network are in chip targets
pou_dt <- pou_nt[, .(target_gene, sample)][, network:=.N, target_gene]
pou_dt[network == 2, sample := "Ncol&Elav"]
pou_dt[, chip := target_gene %in% pou_pub_target_genes]
pou_dt[, sample := str_remove(sample, "_pos")]
pou_dt[, sample := factor(sample, levels = c("Ncol", "Ncol&Elav", "Elav"))]
pou_dt[, label := paste0(sample, "_chip_", chip)]
pou_dt <- unique(pou_dt)

# add mutant data from second paper (parsed below)
pou_mut <- fread(file.path(pub_dir, "pou4_targets_cellrep.tsv"))
pou_dt[target_gene %in% pou_mut$target_gene, .N, label]
pou_dt <- merge.data.table(
  pou_dt, pou_mut[, .(target_gene, regulation)],
  by = "target_gene", all.x = TRUE, sort = FALSE
)
pou_dt[is.na(regulation), regulation := "none"]
pou_dt[, regulation := factor(regulation, levels = c("down", "up", "none"))]

# barplot of target genes
gp_bar <- ggplot(pou_dt, aes(sample, fill = label, color = regulation, linetype = regulation)) +
  geom_bar(size = 0.25) +
  scale_fill_manual(values = c(
...
```{r include=FALSE, eval=TRUE}
pdf(
  file.path(fig_dir, "Pou4_targets_ovl_barplot_elife.pdf"),
  width = 7, height = 5
)
print(gp_bar)
dev.off()
...
pou_dt <- pou_dt[!target_gene %in% pou_dt[!is.na(up) & !is.na(down)]$target_gene]
...
p1 <- plot(
  fit,
  quantities = TRUE,
  fills = list(fill = c("#b69e2eff", "#92c5de", "#e5998d"))
)
print(p1)
