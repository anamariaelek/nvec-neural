...
gp_pou_diff <- ggplot(sub_dt, aes(target_expression_lfc, motif_score, color = regulation)) +
  geom_point() +
  scale_color_manual(values = pou_cols) +
  # geom_hline(yintercept = 4, linetype = 2) + # footprint score
...
# motif scores
mta_annt <- fread("ATACSEQ/nucleosome_free_regions/results/motif-scores-archetypes-mona-annot.tsv")
setnames(mta_annt, "motif", "motif_name")
mta_annt[, motif := str_extract(motif_name, "ARCH\\d+")]
mta_annt[motif == "ARCH1151"]
setnames(mta_annt, "gene", "target_gene")
mta_annt <- mta_annt[order(-score)][, .SD[1], .(motif, target_gene)]
mta_annt <- mta_annt[target_gene != ""]
mta_annt <- mta_annt[, .(motif, score, target_gene)]
setnames(mta_annt, "score", "max_motif_score")

pou_mx <- mta_annt[motif == "ARCH1151"]
mta_dt <- merge.data.table(sub_dt, pou_mx, all.x = TRUE)
