...
gp_acc <- ggplot(norm_dt, aes(sample, log10(norm_counts), fill=reporterline)) +
...
gene_order <- unlist(gene_order_list[cluster_order])
...
module_ann <- HeatmapAnnotation(
    ...
    # "mcol" = modules_dt[match(rownames(plot_mt),gene)]$modules,
    "module" = anno_block(
      labels = unique(modules_dt[match(rownames(plot_mt),gene)]$modules),
      gp = gpar(col=NA)
    )
)
...
hm <- Heatmap(
  ...
  row_title = "genes", 
  row_split = modules_dt[match(rownames(plot_mt),gene)]$modules, 
