...
con_dt <- melt.data.table(
  con_dt,
  id.vars = "gene",
  variable.name = "sample",
  value.name = "expr"
)
con_dt <- merge.data.table(
  con_dt, col_dt,
  by = "sample",
  all.x = TRUE,
  sort = FALSE
...
```{r fig.height=10, fig.width=8, warning=FALSE, message=FALSE}
# Motifs dictionary
dict <- fread("ATACSEQ/nucleosome_free_regions/results/motif-archetypes-PPM-PCCnorm-0.8-IC0.5-8bp-mapped.dict")
dict_mot <- unique(dict[, .(archetype_name, gene)][gene != ""])
setnames(dict_mot, c("motif", "gene"))
# Diff motifs
bnd_dt <- fread("ATACSEQ/nucleosome_free_regions/results/diff-motif-binding-scores-archetypes-mona-annot-footprints.tsv.gz")
pval_thr <- 0.05
scor_thr <- 0.02
bnd_dt <- bnd_dt[diff_bind_pval < 0.05][abs(diff_bind_score) > 0.01]
bndg_dt <- merge.data.table(bnd_dt, dict_mot, by = "motif")
bndg_dt[, lvl1 := strsplit(comparison, "_vs_")[[1]][1], by = 1:nrow(bndg_dt)]
bndg_dt[, lvl2 := strsplit(comparison, "_vs_")[[1]][2], by = 1:nrow(bndg_dt)]
bndg_dt[, group := paste(lvl1, lvl2, sep = " vs ")]
bndg_dt[diff_bind_score < 0, group := paste(lvl2, lvl1, sep = " vs ")]
#bndg_dt[, reporterline := str_extract(group, "Elav|Ncol|Fox")]
#bndg_dt[, condition := str_extract(group, "pos|neg")]
...
mark_ann <- HeatmapAnnotation(
    ...
    "Fox vs Elav" = rownames(plot_mt) %in% bndg_dt[group == "Fox_pos vs Elav_pos"]$gene,
    "Fox vs Ncol" = rownames(plot_mt) %in% bndg_dt[group == "Fox_pos vs Ncol_pos"]$gene,
    "Elav vs Fox" = rownames(plot_mt) %in% bndg_dt[group == "Elav_pos vs Fox_pos"]$gene,
    "Elav vs Ncol" = rownames(plot_mt) %in% bndg_dt[group == "Elav_pos vs Ncol_pos"]$gene,
    "Ncol vs Fox" = rownames(plot_mt) %in% bndg_dt[group == "Ncol_pos vs Fox_pos"]$gene,
    "Ncol vs Elav" = rownames(plot_mt) %in% bndg_dt[group == "Ncol_pos vs Elav_pos"]$gene,
    "marker" = anno_mark(
      at = row_labels_marks_ids,
      labels = row_labels_marks,
