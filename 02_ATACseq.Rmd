...
norm_mt <- (pks_mt+10)/apply(pks_mt+10,1,median) 
...
```{r}
# only CisBP
# cisbp_dir <- file.path("CisBP","CisBP_2023_01_26")
# cisbp_tfs <- fread(file.path(cisbp_dir, "TF_Information.txt"), fill = TRUE)
# cisbp_mot <- read_cisbp(file.path(cisbp_dir, "PWM.txt"))

# CisBP motifs + aditionally transferred based on %ID
pwms_files_cisbp <- file.path("annotation","CisBP_ident_transfered_motifs.RDS")
# scRNAseq_nvec_v3/TFs/CisBP/Nvec/CisBP_ident_transfered_motifs.RDS
pwms_cisbp <- readRDS(pwms_files_cisbp) # 2382

# annotation for CisBP motifs
...
pwms <- readRDS(file.path(res_dir,"motifs-all.rds"))
sim_mat <- readRDS(file.path(sim_dir,sprintf("motifs-similarity-%s-%s%s.rds",similarity,method,normalization)))
...
# list of 1143 archetypes including 3392 motifs (137 archetype(s) fail filters)
arch <- arch[sapply(arch, function(x) length(x)>0)]
arch_file <- file.path(sim_dir,sprintf("motif-archetypes-%s-%s%s-%s-IC%s-%sbp.rds",similarity,method,normalization,min_cluster_similarity,IC_threshold,len_threshold))
...
setnames(cisbp_ann,"Motif_ID","motif")
...
arch_file <- file.path(
  res_dir,
  sprintf("motif-archetypes-%s-%s%s-%s-IC%s-%sbp.rds",similarity,method,normalization,min_cluster_similarity,IC_threshold,len_threshold)
)
arch <- readRDS(arch_file)
...
heatmap_file <- file.path(
  fig_dir,
  sprintf("motif-similarity-archetypes-%s-%s%s-%s-IC%s-%sbp.pdf",similarity,method,normalization,min_cluster_similarity,IC_threshold,len_threshold)
)
