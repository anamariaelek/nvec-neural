...
pdf(file.path(fig_dir, "peaks_var.pdf"), width=8, height=6)
gp_pch
dev.off()
```
...
cluster_names <- structure(paste0("M",seq_along(cluster_order)), names = cluster_order)
peaks_order <- unlist(peaks_order_list[cluster_order])
modules_dt <- data.table(
  peak = unlist(peaks_order_list),
  clusters = as.character(rep(names(peaks_order_list), sapply(peaks_order_list, length)))
)[,modules:=cluster_names[as.character(clusters)]]
```
...
module_ann <- HeatmapAnnotation(
    ...
    # "mcol" = modules_dt[match(rownames(plot_mt),peak)]$modules,
    "module" = anno_block(
      labels = unique(modules_dt[match(rownames(plot_mt),peak)]$modules),
      gp = gpar(col=NA)
    )
)
...
hm <- Heatmap(
  ...
  row_title = "peaks", 
  row_split = modules_dt[match(rownames(plot_mt),peak)]$modules, 
...
```
```{r include=FALSE, eval=TRUE}
pdf(file.path(fig_dir, "peaks_heatmap.pdf"), width=8, height=10)
draw(hm)
dev.off()
```
