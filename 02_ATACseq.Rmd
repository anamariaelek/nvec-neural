...
do
  ...
  TOBIAS BINDetect \
    --motifs ${motifs} \
    --signals \
      ${bwg_dir}/${line}_pos_footprints.bw \
      ${bwg_dir}/${line}_neg_footprints.bw \
    --genome ${genome} \
    --peaks ${peaks} \
    --outdir ${out_dir}/${line} \
...
gpv_list <- lapply(names(comparas), function(x) {
  dt <- unique(comparas[[x]][, .(motif, diff_bind_score, diff_bind_pval)])
  setnames(
    dt,
...
gpv <- wrap_plots(gpv_list, nrow = 2)
gpv
```

```{r include=FALSE, eval=TRUE}
pdf(
  file.path(fig_dir, "motifs-footprint-diff.pdf"),
  width = 18, height = 12
)
gpv
dev.off()
