...
# debugging
# id <- seqnames(ur)=="NC_064034.1" & start(ur)>3160976 & end(ur)<3168925
# ur <- ur[id]
# up <- up[id]
...
# col_dt[,linecondition:=paste(as.character(reporterline), as.character(condition), sep="-"), by=1:nrow(col_dt)]
# col_dt[,linecondition:=factor(linecondition)]
...
```{r}
ggplotMA <- function(res, padj_thr=0.05,sign_col="red",lims=c(NA,NA),title="") {
  res_dt <- as.data.table(res)
  res_dt[log2FoldChange<lims[1], log2FoldChange:=lims[1]]
  res_dt[log2FoldChange>lims[2], log2FoldChange:=lims[2]]
  gp <- ggplot(res_dt, aes(x=baseMean, y=log2FoldChange, colour=padj<padj_thr)) + 
    ...
    scale_y_continuous(limits = lims, expand = expansion(mult = c(0.01,0.01))) +
...
res_pos_neg <- results(dds, contrast=c("condition","positive","negative"))
coef <- "condition_positive_vs_negative"
res_lfc_pos_neg <- lfcShrink(dds, coef=coef, type="apeglm")
gp_pos_neg <- ggplotMA(res_lfc_pos_neg, lims=c(-2,2), title=str_replace_all(coef, c("_"=" ","condition"="Condition:")))
...
1. Differential peaks between conditions
2. Shared peaks with different footprint scores
