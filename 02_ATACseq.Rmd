...
g <- "Nvec_vc1.1_XM_032385706.2"
assign[gene == g, .(gene, peak, is_promoter)]
...
For DESeq2, calculate scores as sums of raw peak counts (they have to be raw counrs andintegers). 
...
```{r fig.height=4, fig.width=12, message=FALSE, warning=FALSE, echo=FALSE}
# TF families
TF_family_annotation_file <- file.path("annotation","gene_families_searchinfo.csv")
...
orp_arc_dt <- unique(orp_arc_dt[, .(archetype, tf_family)])
...
Parse motif enrichment for plotting
...
```{r include=FALSE, eval=TRUE}
pdf(file.path(fig_dir, "motifs_heatmap.pdf"), width=12, height=16)
gp
dev.off()
```
