...
# have to split motifs in three files because of open file limit
for x in {1..6}
do
...
bind_dt <- rbindlist(lapply(1:6, function(x) {
  fn <- list.files(
    file.path("ATACSEQ/nucleosome_free_regions/footprint/BINDetect/", x),
...
fwrite(bind_dt, file.path(
  "ATACSEQ/nucleosome_free_regions/footprint/BINDetect/bindetect_results.txt"
), sep = "\t")
```
...
gpv <- (wrap_plots(gpv_list, nrow = 1) & theme(legend.position = "bottom")) +
...
# heatmaps
hm_dt <- rbindlist(lapply(comparas, function(x) {
  cols <- c(
    "motif_id",
    sprintf("%s_%s_change", x[[1]], x[[2]]),
    sprintf("%s_%s_pvalue", x[[1]], x[[2]])
  ...
  setnames(dt, c("motif", "log2FoldChange", "pvalue"))
  setorder(dt, pvalue)
  dt[, motif_label := strtrim(motif, 30)]
  dt[, motif_label := factor(motif_label, levels = unique(dt$motif_label))]
  dt[, compara := sprintf("%s_%s", x[[1]], x[[2]])]
  dt
}))
hm_dt[, minuslog10pvalue := -1 * log10(pvalue)]
mots <- hm_dt[abs(log2FoldChange) > 0.2]$motif
hm_dt <- hm_dt[motif %in% mots]

gpv_dt <- dcast.data.table(
  hm_dt, motif ~ compara, value.var = "log2FoldChange"
)
gpv_mt <- as.matrix(gpv_dt[, -1])
rownames(gpv_mt) <- unique(gpv_dt$motif)
mord <- rev(order(apply(gpv_mt, 1, which.max)))
hm_dt[, motif := factor(motif, levels = rownames(gpv_mt)[mord])]
setorder(hm_dt, motif)

gp_dot <- ggplot(hm_dt, aes(
  compara, motif_label,
