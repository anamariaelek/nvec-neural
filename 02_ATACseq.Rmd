...
fwrite(
  dict_new,
  file.path(atac_dir, "motif-archetypes-PPM-PCCnorm-0.8-IC0.5-8bp-mapped.dict"),
  sep = "\t"
)
```
...
hm_dt <- fread(file.path(bind_dir, "bindetect_results.txt"))
...
# motifs dictionary
# to order motifs by structural class
dict <- fread(file.path(
    res_dir, "motif-archetypes-PPM-PCCnorm-0.8-IC0.5-8bp-mapped.dict"
))
dict_mot <- unique(dict[, .(archetype_name, tf_family, tf_class)])
setnames(dict_mot, "archetype_name", "motif")
grep_pairs <- list(
  "fox" = "Forkhead",
  "gata" = "GATA",
  "hox" = "Homeodomains",
...
for (i in seq_along(grep_pairs)) {
  dict_mot[
    grepl(names(grep_pairs)[i], motif, ignore.case = TRUE) & tf_family == "",
    tf_family := grep_pairs[i]
  ]
}
...
First subset targets for only expressed genes.
...
# subset for expressed genes in each reporter line

# tpm gene expression
tpm_mt <- read.table(
  file.path("RNASEQ_QUANTIFICATION", "results", "tpm_expression.tsv")
)
...
mo_dt[, expressed := FALSE][tpm > 0.5, expressed := TRUE]
unique(
...
hits_dt <- unique(mo_dt[expressed == TRUE][, .(sample, motif, gene)])
...
Inspect number of target genes overlapping in different samples
```{r}
...
# # best correlated motifs
mots_dt <- fread(file.path("results", "correlation_expression_binding_posterboys.tsv"))
mots <- mots_dt$motif

mo_dt <- fread(file.path(res_dir, "motifs_genes_targets_long.tsv.gz"))

# plot upset diagrams
up_gps <- lapply(mots, function(mo) {
  mo_df <- dcast.data.table(
    mo_dt[expressed == TRUE][motif == mo][, .(gene, sample)][, N := 1],
...
```{r include=FALSE, eval=TRUE}
pdf(
  file.path("plots", "BINDetect_motif_target_hits_upset.pdf"),
  width = 10, height = 7
)
print(up_gps)
dev.off()
...
```{r}
# parse per sample and add gene annotation
mo_dcs <- dcast.data.table(
  mo_dt[, .(motif, gene, sample)][, N := 1],
  motif + gene ~ sample, value.var = "N"
)
mo_dcs[is.na(mo_dcs)] <- 0
mo_ann <- merge.data.table(
  mo_dcs, gene_ann[, .SD[1], gene],
  by = "gene", all.x = TRUE, sort = FALSE
)
mo_ann[is.na(pfam), pfam := ""]
mo_ann[is.na(name), name := ""]
setcolorder(mo_ann, c("motif", "gene", "pfam", "name"))
fwrite(
  mo_ann,
  file.path(res_dir, "motifs_genes_targets_wide_expressed.tsv.gz"),
  sep = "\t"
)

# targets only in positive samples
mo_one <- mo_ann[motif == "ARCH2465_POU1F1_POU4F3_POU4F2"]
mo_pos <- mo_one[Elav_neg == 0 & Fox_neg == 0 & Ncol_neg == 0]
mo_pos[Elav_pos == 1 & Fox_pos == 1 & Ncol_pos == 1]
mo_pos[Elav_pos == 1 & Fox_pos == 0 & Ncol_pos == 0]
mo_pos[Elav_pos == 0 & Fox_pos == 1 & Ncol_pos == 0]
mo_pos[Elav_pos == 0 & Fox_pos == 0 & Ncol_pos == 1]

mo_pos[motif == "ARCH1154_RFX_RFX2"]
```
